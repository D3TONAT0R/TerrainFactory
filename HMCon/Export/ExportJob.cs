...
namespace HMCon.Export {
	public class ExportJob {
		...
		public void Export() {
			if(format != null && format.exportHandler != null) {
				format.exportHandler.Export(this);
			} else {
				if(format != null) {
					throw new ArgumentException($"No exporter is defined for format '{format.Identifier}'");
				} else {
					throw new NullReferenceException("FileFormat is null!");
				}
			}
		}
	}
