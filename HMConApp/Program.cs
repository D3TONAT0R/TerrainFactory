...
namespace HMConApp {
	public class Program {
		...
		public static void Main(string[] launchArgs) {
			...
			HMConManager.Initialize(loadPlugins ? AppContext.BaseDirectory : null);

#if DEBUG
			if(launchArgs.Length > 0 && launchArgs[0] == "auto") HMConManager.autoInputActive = true;
#endif
			WriteLine("---------------------------------");
		...
		static bool? HandleCommand(string cmd, string[] args, bool batch) {
			...
			foreach(var c in CommandHandler.GetConsoleCommands()) {
				if(c.command == cmd) {
					c.commandHandler.HandleCommand(c.command, args, currentJob.exportSettings, currentJob.CurrentData);
					return null;
				}
			}
			if(batch) {
